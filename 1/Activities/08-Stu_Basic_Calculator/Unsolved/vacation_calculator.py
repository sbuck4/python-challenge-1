...
budget_left = (budget - flights_accommodation)
print("You have", budget_left, "per day in the local currency")

# Calculate the remaining budget in local currency amount
budget_remain = (budget_left * exchange_rate)
print("you have" , budget_remain , "per day in local currency")

# Calculate the budget per day in the local currency
budget_per_day = budget_remain / vacation_days
print("You can spend", budget_per_day , "per day in local currency" )

# Calculate the budget per day in the local currency, ignoring cents
budget_pd = budget_left // vacation_days
